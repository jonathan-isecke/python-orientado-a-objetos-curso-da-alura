...
assistiram.extend(usuarios_machine_learning)  # .extend()  →  Só adiciona os elementos de uma lista em outra, copiando
print('Lista extendida:', assistiram)

set(assistiram)  # set() de Conjuntos, da Matemática. Retorna os elementos da intersecção de conjuntos (sem repetições)

print('Conjunto: ', set(assistiram))

# OBS: Se a ordem não importa e queremos elementos não repetidos, podemos usar os conjuntos (set).
# O set não considera a ordem de entrada (passada pelo desenvolvedor ou usuário). Às vezes ordena em ordem crescente...
# set() não possui indexação (índice) como as listas. Então não tem como acessar por posição: 'set(usuarios[2])'
# Conjuntos não tem ordenação específica. Imagine quando vc desenha um conjunto circular com vários números dentro...
...
print('União:', usuarios_data_science | usuarios_machine_learning)  # '|' (barra vertical ou pipe) = está em 1 OU o outro (Quero ver o que aparece em pelo menos 1 dos conjuntos)
print('Intersecção:', usuarios_data_science & usuarios_machine_learning)  # '&' = está em 1 E está no outro (Quero ver só o que aparece nos 2 conjuntos)
print('A, exceto B:', usuarios_data_science - usuarios_machine_learning)
print('B, exceto A:', usuarios_machine_learning - usuarios_data_science)
print('Exclusivo de cada conjunto:', usuarios_machine_learning ^ usuarios_data_science)  # Todos os elementos, exceto os de intersecção


# Podemos alterar conjuntos em tempo real
print('\nUSERS')
usuarios = {1, 5, 76, 34, 52, 13, 17}
print('Len:',len(usuarios))
usuarios.add(765)  # add() é equivalente ao append() das listas. O add() ñ adiciona no final pois não há ordenação em conjuntos

usuarios = frozenset(usuarios)  # frozenset() trava o conjunto contra alterações
# usuarios.add(765)  # Isso não vai funcionar mais porque congelamos o conjunto. Então não dá para fazermos mais alterações.


meu_texto = "Bem vindo meu nome é Guilherme eu gosto muito de nomes e tenho o meu cachorro e gosto muito de cachorro"
meu_texto = meu_texto.split()  # Por padrão, o .split() já considera o espaço em branco " "
print(meu_texto)
print("Palavras sem repetições:", set(meu_texto))
...
del aparicoes["Carlos"]  # Removendo itens de um dicionário

print("cachorro" in aparicoes)  # Checagem se o valor 'cahorro' está dentro das chaves de 'aparicoes'
...
print('\nTeste 2')
for elemento in aparicoes.keys():  # Passe pelas chaves primeiro
    print(elemento)

print('\nTeste 3')
for elemento in aparicoes.values():  # Passe pelos valores primeiro
    print(elemento)
...
print('\nTeste 5')
for chave, valor in aparicoes.items():  # Estamos desempacotando chave e valor
    print(chave, "=", valor)

print('\nTeste 6')
print(["palavra {}".format(chave) for chave in aparicoes.keys()])  # List Comprehension
print("--"*50)
...
for palavra in meu_texto.split():
    ate_agora = aparicoes.get(palavra, 0)  # Procure a palavra 'palavra'. Se não estiver no dicionário, devolva '0'
    aparicoes[palavra] = ate_agora + 1

print(aparicoes)
# OBS: Passar o valor 0 em um dicionário quando não encontramos um valor é tão comum que foi criado um método para isso:


# DEFAULT DICT
from collections import defaultdict

aparicoes = defaultdict(int)  # O método int() por padrão devolve 0 quando não é passado nenhum valor para ele.
# Estamos falando basicamente para criar um dicionário que atribua o valor zero como padrão caso não encontre o valor.

for palavra in meu_texto.split():
    ate_agora = aparicoes[palavra]  # Vai devolver o '0' novamente se não encontrar o valor por causa do 'defaultdict'
    aparicoes[palavra] = ate_agora + 1
...
print(contas[15])
# Neste exemplo acima, toda vez que eu chamar um objeto classe Conta e ele não existir, ele vai criar um novo objeto.
